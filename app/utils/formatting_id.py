...
def generate_single_pkey_id(
    ...
    <indicator>_<company_id>_<year>_XXX where XXX is a zero-padded sequence.
    """
    sequence_length = 3
    id_column = getattr(model_class, id_field)

    pattern = f"{indicator}-{company_id}-{year}-%"
    last_entry = (
    ...
    generated_id = f"{indicator}-{company_id}-{year}-{next_sequence}"
...
def generate_bulk_pkey_ids(
    ...
    for i in range(count):
        ...
        generated_id = f"{indicator}-{company_id}-{year}-{sequence_num:0{sequence_length}d}"
